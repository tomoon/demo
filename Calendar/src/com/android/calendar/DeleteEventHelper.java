...
public class DeleteEventHelper {
    ...
    private void deleteStarted() {
        // add by hhyuan to notify sync when delete on watch
               notifyIndroidSyncDelete();
    ...
    public void dismissAlertDialog() {
       ...
       // hhyuan add for notify indroid sync
       private void notifyIndroidSyncDelete() {
               if (mModel != null) {
                       AsyncQueryService queryService = new AsyncQueryService(mContext) {
                               ...
                               protected void onQueryComplete(int token, Object cookie,
                                       ...
                                       cursor.moveToFirst();
                                       int indroidSyncId = -1;
                                       indroidSyncId = cursor.getInt(2);
                                        cursor.close();

                                       if (indroidSyncId < 0)
                                               return;
                                       Intent intent = new Intent();
                                       intent.setAction("com.android.calendar.action_delete_event_from_watch");
                                       intent.putExtra("indroidSyncId", indroidSyncId);
                       ...
                       Uri uri = Uri.parse("content://" + CalendarContract.AUTHORITY
                                       + "/indroid_sync");
                       String[] projection = new String[] { "_id", "event_id",
                                       "indroid_sync_id", };
                       String selections = "event_id = ?";
                       String[] args = new String[] { String.valueOf(mModel.mId) };
                       // String order = " indroid_sync_id DESC ";
                       queryService.startQuery(queryService.getNextToken(), null, uri,
                                       projection, selections, args, null);
               }
