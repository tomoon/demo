...
public class MediaPlaybackService extends Service {
    ...
    public static final int LRC_CLOSE = 0;
    public static final int LRC_OPEN = 1;
    ...
    private int mLrcStatus = LRC_OPEN;
    ...
    private boolean teleComing = false;
    private TelephonyManager telephonyManager;
    private TelephoneListener telephoneListener = new TelephoneListener();//lnliu add

    private RemoteControlClient mRemoteControlClient;
    private boolean mUsbClose;
...
//        mRemoteControlClient.setTransportControlFlags(flags);
    ...
    public void onDestroy() {
        ...
        telephonyManager.listen(telephoneListener, PhoneStateListener.LISTEN_NONE);//lnliu add
  ...
  //lnliu add start for phone
    class TelephoneListener extends PhoneStateListener {
    	...
    	public void onCallStateChanged(int state, String incomingNumber) {
    		...
    		if(state != TelephonyManager.CALL_STATE_IDLE){
    			if(isPlaying() && !teleComing) {
            		Log.e(LOGTAG,"isPlaying , so pause and teleComing is true");
            		teleComing = true;
            		pause();
            	}
    		} else {
    			if(!isPlaying() && teleComing){
    				Log.e(LOGTAG,"Playing, and teleComing is false");
    				teleComing = false;
                    play();
    			}
    		}
    ...
    private void saveQueue(boolean full) {
        ...
        ed.putInt("lrcstatus", mLrcStatus);
    ...
    private void reloadQueue() {
        ...
        if (qlen > 1) {
            ...
            mLrcStatus = mPreferences.getInt("lrcstatus", LRC_OPEN);
        }
    }
    ...
    public void registerExternalStorageListener() {
        if (mUnmountReceiver == null) {
            mUnmountReceiver = new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    String action = intent.getAction();
                    String tmpstoragePath = intent.getData().getPath();
                    if (action.equals(Intent.ACTION_MEDIA_EJECT)) {
                        Log.d(LOGTAG,"MEDIA_EJECT--tmpstoragePath:"+tmpstoragePath+" getFileName():"+getFileName());          
                        mUsbClose=true;       
                        //if(tmpstoragePath.startsWith(getFileName())){
                        if(getFileName().startsWith(tmpstoragePath)){
                            Log.v(LOGTAG,"--stopplay");
                        saveQueue(true);
                        mQueueIsSaveable = false;
                            closeExternalStorageFiles(tmpstoragePath);
                        }                      
                    } else if (action.equals(Intent.ACTION_MEDIA_MOUNTED)) {
                        Log.d(LOGTAG,"MEDIA_MOUNTED--tmpstoragePath:"+tmpstoragePath+" getFileName():"+getFileName());
                        mUsbClose=false;
                        //if(tmpstoragePath.startsWith(getFileName())){
                        if(getFileName().startsWith(tmpstoragePath)){
                            Log.v(LOGTAG,"--reloadQueue");
                        mMediaMountedCount++;
...
//            ed.apply();
            ...
            if (! mPlayer.isInitialized()) {
                ...
                if (! mPlayer.isInitialized() && mOpenFailedCounter != 0) {
                    ...
                    mOpenFailedCounter = 0;
                     if(mUsbClose)
                    {
                    	Toast.makeText(this, R.string.usb_busy, Toast.LENGTH_LONG).show();
                    }
                    else
                    {
                    if (!mQuietMode) {
    ...
    static class ServiceStub extends IMediaPlaybackService.Stub {
		...
		public void setLrcStatus(int lrcStatus){		
			mService.get().setLrcStatus(lrcStatus);
		}


		public int getLrcStatus() {
			return mService.get().getLrcStatus();
		}
